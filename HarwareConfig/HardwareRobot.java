...
//    public ColorSensor lightSensor = null;
    ...
    public FieldLocation robotLocation = new FieldLocation(0,0,0);
...
//            servoFoundationR = new Servo();
            ...
            imu = new OfflineCode.OfflineHW.BNO055IMU();
...
//            servoCapstoneRelease = hwMap.get(Servo.class, "capstone_servo");
    ...
    public void initMiniBot(HardwareMap ahwMap, boolean tm) {
        ...
        if(tm) {
            ...
            /** Need below if running autonomous program that is limited to miniBot
             *
             */
...
//        clockwise = (g1.right_stick_x) * om.cons.TELEOP_ROTATE_POWER_LIMIT;
    ...
    public void drivePursuit(ArrayList<PursuitPoint> pathPoints, BasicAuto om, String step){
        ...
        targetPoint = findPursuitPoint(pathPoints, robotLocation, radius);
        ...
        while((count < 295) && (om.opModeIsActive() || om.testModeActive)) {
            ...
            setRobotAngle();
            distanceToTarget = findDistance(targetPoint,new PursuitPoint(robotLocation.x, robotLocation.y));
...
//Angle reference from field to robot is negative angle in CW = + robot frame, uses + = CCW {IMU frame & field frame}
        ...
        fieldX += fieldXInc;
        fieldY += fieldYInc;
        fieldAngle = -robotAngle;//IMU field angle should be same sign as used for theta in rotation matrix, but this is returning "-"
